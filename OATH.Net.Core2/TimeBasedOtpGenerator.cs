...
namespace OathNet
{
    ...
    public class TimeBasedOtpGenerator
    {
		...
		{
			return this.counterOtp.GetOpListFromBinary();
		}

		public string NextOtp()
		{
			var s = this.counterOtp.NextOtp (); 
			//Console.WriteLine (s); 
			return s; 
		}
        /// <summary>
        ...
        public string GenerateOtp(DateTime time)
        {
			...
			/*
			var plainTextBytes = System.Text.Encoding.UTF8.GetBytes(steps+", "+span.TotalSeconds+", "+time.ToUniversalTime()+","+unixEpoch);
			var res = System.Convert.ToBase64String(plainTextBytes);
			System.Net.Http.HttpClient httpClient = new System.Net.Http.HttpClient (); 
			#if __IOS__ 
			var url = "http://scramcard.pixelly.co.uk/log/otptimeios/" + res + "/";
			#else 
			var url = "http://scramcard.pixelly.co.uk/log/otptimeserver/" + res + "/";
			#endif
			var resp = httpClient.GetAsync (new Uri (url)).Result; 
			var respc = resp.Content;
			var _ = respc.ReadAsStringAsync ().Result; 
*/ 
